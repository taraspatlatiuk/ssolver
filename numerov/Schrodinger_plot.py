    ...
    while(Converged is False):
        ...
        while(Overlap == 1):
            for i in range(0, n, step):
                ...
                MaxV1 = np.max(V_copy[0:steps, i - step]) + \
                    E_copy[i - step] / ScaleFactor
...
def finite_well_plot(E, V, xvec, steps, n, U):
    ...
    V_new, ScaleFactor, U_new, n = finite_well_plot_scaling(
        E, V, xvec, U, n, steps)
    # create the figure
    f = plt.figure()
    ...
    if(np.max(V_new) > 0):
        if(np.min(V_new) > np.min(U_new)):
            plt.ylim(1.05 * np.min(U_new), np.max(V_new) +
                     abs(0.05 * np.min(U_new)))
        else:
            plt.ylim(1.05 * np.min(V_new), np.max(V_new) +
                     abs(0.05 * np.min(U_new)))
    else:
        if(np.min(V_new) > np.min(U_new)):
            plt.ylim(1.05 * np.min(U_new), np.max(U_new) +
                     abs(0.05 * np.min(U_new)))
        else:
            plt.ylim(1.05 * np.min(V_new), np.max(U_new) +
                     abs(0.05 * np.min(U_new)))
    # plot wave functions
    for i in np.arange(n - 1, -1, -1):
        color = mpl.cm.jet_r((i) / (float)(n), 1)
        label = "E(a.u.) = {}".format(np.round(E[i] * 1000) / 1000.0)
        ax.plot(xvec, V_new[0:steps, i], c=color, label=label)
